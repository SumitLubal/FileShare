...
public class Client {
	...
	public void sendMultiFiles(File fil) throws IOException {
		...
		dos.writeInt(count);
		System.out.println(Test.sendToCluster);
		if (Test.sendToCluster) {
			dos.writeUTF(loginframework.Frame.userName); // please recive this
			System.out.println(loginframework.Frame.userName);
	...
	public void send(File fil, String path) throws Exception {
		try {
			...
			if (fil.isFile()) {
				...
				if(Test.sendToCluster){
					System.out.println("Sending username to cluster" + loginframework.Frame.userName);
					dos.writeUTF(loginframework.Frame.userName);
				}
				
				dos.writeUTF(path);
				System.out.println("Path sent is: " + path);
				dos.writeUTF(name);
